...
var Date_RExpr=/^\d\d\d\d\/\d\d\/\d\d$/;
var nationalcode_RExpr=/^\d\d\d\d\d\d\d\d\d\d$/;
var mobile_RExpr= /09\d\d\d\d\d\d\d\d\d$/;
...
function checkNationalcode(field, rules, i, options){
	if(field.val()!="")	
	if(!nationalcode_RExpr.test(field.val()))
		return options.allrules.checknationalcode.alertText;
}

function checkmobile(field, rules, i, options){
	if(field.val()!="")	
	if(!mobile_RExpr.test(field.val()))
		return options.allrules.checkmobile.alertText;
}
...
function checkDate(field, rules, i, options){
	if(field.val()!="")	
	if(!Date_RExpr.test(field.val()))
		return options.allrules.checkDate.alertText;
}
...
function mustnobezero(field, rules, i, options){
	if(field.val()!="")
	if(!intNumber_RExpr.test(field.val()))
		return options.allrules.checkIntNumber.alertText;
	else if(field.val()<1)
			return options.allrules.mustnobezero.alertText;
